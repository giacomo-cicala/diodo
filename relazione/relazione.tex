...
\begin{figure}[h!]
	...
	\begin{circuitikz}
		...
		++(1.5,0) coordinate(oscope_up) to[oscope, v=$v$] (oscope_up |- GND) coordinate(oscope_down) to [short, -*] (pot_down);

		\draw (oscope_up) to[short, *-] ++(2,0) coordinate (vmeter_up) to[rmeterwa, t=V, v=$v$] (vmeter_up |- GND) to [short, -*] (oscope_down);
	\end{circuitikz}
	\hfill
	\begin{circuitikz}
		% Generatore
...
(\cref{fig:alimentatore}) impostato a $+5$V, un potenziometro da 1 k$\Omega$ e
due diodi a giunzione p-n, uno al silicio (\err{codice diodo?}) e uno al
germanio (\err{uguale}). La tensione ai capi del diodo e la corrente che lo
attraversa sono state misurate rispettivamente con un oscilloscopio da banco
\texttt{GW Instek GOS-652G} (\cref{fig:oscilloscopio}) e da un multimetro
\texttt{Fluke 175 True RMS Multimeter} (\cref{fig:multimetro}) in modalità
